...
MAX_STEP = 200                          #每局最大步数
SHOW_EVERY = int(EPISODE_N*0.09)         #表格平滑窗口
##########################################################################
...
class envCube:  # 生成环境类
    ...
    NUM_ENEMIES = 3   # enemy的数量
    distances = 0       #记录agent和food的距离之和
        ...
        if self.RETURN_IMAGE:               #通过图像获取state
            state = np.array(self.get_image())
                ...
                for i in range(self.NUM_PLAYERS):
                    ...
                    self.distances = np.linalg.norm(state, ord=1)     #计算agent和food的距离
                    # 考虑和每个enemy的位置关系
                    for j in range(self.NUM_ENEMIES):
